                            ...
                            <div className="flex flex-wrap justify-center">
                                <div className="flex w-full justify-center">
                                    <div className="block pt-4 rounded-full border-none align-middle">
                                        <Image src={network.logo} alt={network.name} width={100} height={100} />
                                    </div>
                                </div>
                            </div>
                            <div className="text-center">
                                <h3 className=" text-2xl font-bold leading-normal">{network.name}</h3>
                                <div className=" text-xs font-bold uppercase text-slate-500">mainnet</div>
                                <div className="w-full text-center">
                                    <div className="flex justify-center">
                                        <div className="p-2 text-center">
                                            <span className="block text-xl font-bold uppercase tracking-wide">{network.fee * 100}%</span>
                                            <span className="text-sm text-slate-500">Fee</span>
                                        </div>

                                        <div className="p-2 text-center">
                                            <span className="block text-xl font-bold uppercase tracking-wide">{network.apy ? network.apy * 100 : 'NaN'}%</span>
                                            <span className="text-sm text-slate-500 tooltip" data-tip="Annual Percentage Yield">APY</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="flex justify-center">
                                    <ul>
                                        {network?.validators.map(val => (
                                            <li key={val.address}>
                                                <div className="border p-4 rounded-lg flex bg-gray-50" onMouseLeave={() => setIsCopied(false)}>
                                                    <div className="pr-3 font-light max-w-[16rem] sm:max-w-fit">
                                                        <p className="text-clip truncate">{val.address}</p>
                                                    </div>
                                                    <CopyToClipboard text={val.address} onCopy={() => setIsCopied(true)}>
                                                        <div className="tooltip cursor-pointer" data-tip={isCopied ? 'Copied!' : 'Copy'}>
                                                            
                                                                <svg width={24} height={24} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1">
